...
def coherent_states(n,*args):
    ...
    alph_n = alpha**n
    term = np.sqrt(sp.special.gamma(1+n))
...
def h_m(x,m,n):
    ...
    h1_tst = (-1)**m * sp.special.gamma(n+1) * (2*x)**(n-2*m)
    h2_tst = sp.special.gamma(m+1) * sp.special.gamma((n - 2*m)+1)

    return h1_tst / h2_tst # h1/h2
